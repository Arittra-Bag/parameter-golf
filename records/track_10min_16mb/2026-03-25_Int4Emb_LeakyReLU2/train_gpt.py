...
class Hyperparameters:
    ...
    activation = os.environ.get("ACTIVATION", "leaky_relu2")
...
ACTIVATION_FNS = {
    ...
    "leaky_relu2": lambda x, slope: F.leaky_relu(x, negative_slope=slope).square(),
    "swish2": lambda x: (torch.sigmoid(x) * x).square(),
    "gelu2": lambda x: torch.nn.functional.gelu(x).square(),
}
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, activation, leaky_slope)
...
def main() -> None:
    ...
    log0(f"activation:{args.activation} leaky_relu_slope:{args.leaky_relu_slope}")
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0("sdp_backends:cudnn=False flash=True mem_efficient=False math=False")
    log0("attention_mode:gqa")
